...
export function InspectionsPage() {
  ...
  const columns: ColumnsType<InspectionSample> = [
    ...
    { title: '操作', fixed: 'right', render: (_, row) => <Space><Button size="small" icon={<HistoryOutlined />} onClick={() => void openHistory(row)}>历史</Button>{(row.result === 'pending' || row.retestStatus === 'requested') && <Button size="small" type="primary" icon={<CheckOutlined />} disabled={!can('inspection:write')} onClick={() => setCompleteTarget(row)}>{row.result === 'pending' ? '录入结果' : '再次复测'}</Button>}</Space> },
  ]
  return (
    <div className="page-stack">
      ...
      <Modal title={`${completeTarget?.retestStatus === 'requested' ? '录入复测结果' : '录入结果'} · ${completeTarget?.sampleCode || ''}`} open={Boolean(completeTarget)} confirmLoading={saving} onOk={() => void complete()} onCancel={() => setCompleteTarget(null)} okText="提交" cancelText="取消">
